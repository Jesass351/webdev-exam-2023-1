...
const URLDecodeAddress = "https://catalog.api.2gis.com/3.0/items/geocode";
...
function showMessage(style, message) {
    ...
    alert.classList.add("alert", "alert-dismissible");
    alert.classList.add(`alert-${style}`, "w-100", "m-1");
...
function addGuidesToMainTable(data) {
    ...
    for (let record of data) {
        ...
        avatarDiv.classList.add("align-items-center", "text-center");
        avatarDiv.classList.add("border-end");
...
function createDefaultFilterExp(data) {
    let selectMinExpGuideForm;
    let selectMaxExpGuideForm;

    selectMinExpGuideForm = document.querySelector("#selectMinExpGuideForm");
    selectMaxExpGuideForm = document.querySelector("#selectMaxExpGuideForm");
...
function addRoutesToMainTable(data) {
    ...
    for (let record of data) {
        ...
        if (record.description.length > 50) {
            ...
            descriptionToolTip.setAttribute("data-bs-title", 
                record.description);
            descriptionToolTip.innerText = record.description.slice(0, 50);
            descriptionToolTip.innerText += "...";
        ...
        if (record.mainObject.length > 50) {
            ...
            objectsToolTip.setAttribute("data-bs-title", record.mainObject);
            let mainObjectList = record.mainObject.slice(0, 50).split("-");
            for (let object of mainObjectList) {
                objectsToolTip.innerText += object;
            }
            objectsToolTip.innerText += "...";
        ...
        selectBtnDiv.classList.add("border-end", "p-0");
        let selectBtn = document.createElement('button');
        selectBtn.classList.add("btn", "border-orange");
        selectBtn.classList.add("text-orange", "select-route", "p-1");
        ...
        selectBtn.addEventListener("click", () => {
            getGuidesById(record.id).then(result => {
                ...
                createDefaultFilterLanguage("selectLangGuideForm", 
                    "Язык экскурсии");
            ...
            let routeName = document.querySelector("#routeNameGuideSection");
            routeName.setAttribute("data-id", record.id);
            // showMapSection();
            setCorrectCoordsToMarker(record.coords);
        });

        if (window.screen.width < 450) {
            newRow.classList.add("fs-7");
        }
...
function goToPage(pageNumber) {
    getAllRoutes(pageNumber).then(result => {
        addRoutesToMainTable(result);
    });
    scrollTo(0, 0);
}

function createPaginationBtn(currentPage, specialParam = "notLast") {
    ...
    for (i = start; i <= end; i++) {
        ...
        newPageBtn.addEventListener("click", () => {
            ...
            goToPage(newPageBtn.dataset.page);
        });
...
function searchRoutesByName(searchName) {
    ...
    for (let record of allDataRoutes) {
        if (record.name.includes(searchName) && 
        record.mainObject.includes(object)) {
            resultArray.push(record);
        }
    }
...
function searchRoutesByObjects(objectName) {
    ...
    let resultArray = [];
    for (let record of allDataRoutes) {
        if (record.mainObject.includes(objectName) &&
...
function setTodayDate() {
    ...
    if ((today.getDate() + 1) <= 9) {
        day = "0" + String(today.getDate() + 1);
    } else {
        day = String(today.getDate() + 1);
    }
...
function setRouteNameToModal() {
    ...
    let routeNameGuideSection;
    routeNameGuideSection = document.querySelector("#routeNameGuideSection");
...
function openAddOrderModalHandler(event) {
    ...
    if (!validateGuideSelection()) {
        ...
        let myModal = new bootstrap.Modal(
            document.querySelector("#addOrderFormModal")
        );
        myModal.show();
    }
...
async function addOrderBtnHandler(event) {
    ...
    try {
        ...
        showMessage("success", "Заявка успешно оформлена");
    } catch (error) {
...
async function getCoordinates(address) {
    ...
    let finalURL = new URL(URLDecodeAddress);
...
function lastPageHandler(event) {
    ...
    createPaginationBtn(parseInt(page, 10) + 1, "last");
}

function clearRoutesFilterHandler() {
...
window.onload = function () {
    ...
    let lastPageBtn;
    lastPageBtn = document.querySelector(".last-page-btn");
    lastPageBtn.onclick = lastPageHandler;

    getAllRoutes(0).then(result => {
    ...
    clearRoutesFiltersBtn = document.querySelector("#clearRoutesFiltersBtn");
    clearRoutesFiltersBtn.addEventListener("click", clearRoutesFilterHandler());

    let clearGuidesFiltersBtn;
    clearGuidesFiltersBtn = document.querySelector("#clearGuidesFiltersBtn");
    clearGuidesFiltersBtn.addEventListener("click", () => {
        document.querySelector("#searchGuideForm").reset();
        addGuidesToMainTable(guidesData);
    });

    searchByNameInput = document.querySelector("#searchByNameInput");
    searchByNameInput.oninput = searchByNameInputHandler;

    selectRouteForm = document.querySelector("#selectRouteForm");
    selectRouteForm.onchange = searchRoutesByObjectsHandler;

    selectLangGuideForm = document.querySelector("#selectLangGuideForm");
    selectLangGuideForm.onchange = searchGuidesByLangHandler;

    selectMinExpGuideForm = document.querySelector("#selectMinExpGuideForm");
    selectMinExpGuideForm.onchange = searchGuidesByExpHandler;

    selectMaxExpGuideForm = document.querySelector("#selectMaxExpGuideForm");
    selectMaxExpGuideForm.onchange = searchGuidesByExpHandler;
    ...
    // openAddOrderBtn.addEventListener(
    //     'show.bs.modal', openAddOrderModalHandler
    // );
    ...
    firstPageBtn.onclick = firstPageHandler;
};